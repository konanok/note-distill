...
```bash
# GitHub 安装
/plugin marketplace add konanok/note-distill
/plugin install note-distill@note-distill

# 本地路径安装（开发）
/plugin marketplace add ~/Projects/Github/note-distill
/plugin install note-distill@note-distill
```
...
| `arch` | 架构与设计说明。描述系统/模块/子系统的组件职责、数据流、关键设计决策及已知约束。与 ADR 互补：ADR 记录"为什么选 A 不选 B"，arch 记录"整体长什么样、怎么运转" |
| `investigation` | 技术排查报告。记录 bug / 问题的现象→根因→修复全流程。5 Whys 根因链 + 多方案对比修复。可选段：复现方法、验证方案、遗留风险与待办。frontmatter 含 `follow-up`，支持排查后续跟进方向 |
| `runbook` | 排查操作手册。针对操作不当、数据缺失或参数遗漏导致的问题（非代码缺陷），推导通用诊断步骤和排查路径。含代码线索表格、动态步骤数。与 investigation 互补——investigation 记录代码缺陷，runbook 记录操作流程的排查指引 |
