...
被动记录会话事件，异步分析生成候选知识点。`/note` 时按 hook 覆盖度三态决策：
- **`coverage=full`**（hook 从会话开始就在线）→ primary path，直接用 candidates / event window，无需 `CLAUDE_CODE_FORK_SUBAGENT`
- **`coverage=partial`**（装上 plugin 之前已有大段对话，hook 中途接入）→ fallback path，hook 已捕获片段不可信被忽略，subagent 直接读主会话历史
- **`coverage=empty`**（hook 未启用或未生效）→ fallback path

fallback path 需要在 `.claude/settings.local.json` 的 `env` 字段设 `"CLAUDE_CODE_FORK_SUBAGENT": "1"` 让 subagent 继承主会话上下文。未设置时 subagent 会回报指引提示。
