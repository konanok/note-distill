...
helper 不存在、执行失败或输出为空 → 对应变量 = `unavailable`，COVERAGE = `empty`。

**读取 COVERAGE**：从 `NOTE_EVENT_WINDOW.coverage` 或 `NOTE_CANDIDATES.coverage` 取（两者一致）。可能值：
...
| 条件 | 路径 | 传递素材 |
|---|---|---|
| COVERAGE=`full` 且 NOTE_CANDIDATES/NOTE_EVENT_WINDOW 至少一个含内容 | **primary** | candidates + window，subagent 以此为主 |
| COVERAGE=`partial` | **fallback** | candidates/window 强制设为 `unavailable`（hook 中途接入，已捕获片段不可信），subagent 直接读主会话历史 |
| COVERAGE=`empty` 或 helper 不存在 | **fallback** | 无 hook 数据，subagent 直接读主会话历史 |

> **关键纪律**：COVERAGE=`partial` 时即使 window 已生成、candidates 已有内容，主 agent 也必须把 NOTE_CANDIDATES 和 NOTE_EVENT_WINDOW 改写为字符串 `unavailable` 后再放入 spawn prompt——绝不能传"残缺片段"骗 subagent 当全部素材。

fallback 路径强烈推荐设置 `CLAUDE_CODE_FORK_SUBAGENT=1`（让 subagent 继承完整主会话上下文）。未设置时 subagent 仍会按下方 prompt 中的 fallback 指令尝试读主会话历史，但能力有限——可在回报中提示用户启用 fork。
...
NOTE_CANDIDATES / NOTE_EVENT_WINDOW 不可用（COVERAGE={empty|partial}：hook 未启用、未覆盖
或只捕获到 /note 之后的片段）。素材来源切换为主会话完整对话历史：
- 若 CLAUDE_CODE_FORK_SUBAGENT=1 已生效，你已自动继承主会话上下文，可直接基于上下文判断
- 若未继承上下文，明确回报：`⚠️ 无 hook 数据且未启用 fork，无法访问主会话历史。
  请在 .claude/settings.local.json 的 env 字段添加 "CLAUDE_CODE_FORK_SUBAGENT": "1" 后重试。`
- 按 protocol §1 的增量范围规则确定本次记录范围（找到最近一次 /note 命令，处理两者之间的对话；
  找不到则处理 session 全部）
...
- COVERAGE=partial 时即使 NOTE_EVENT_WINDOW 含数据，也不得作为唯一素材使用
- 按模板重组，严禁流水账转述
- 官方文档/API 附来源 URL
```
