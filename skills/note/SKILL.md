...
1. `$CLAUDE_CODE_SESSION_ID` 存在且不为空字符串 → SESSION_ID = 该值，PLATFORM = `claude-code`（仅 Claude Code 有此环境变量）
2. `{SKILL_DIR}/scripts/find-session.ts --cwd .` → 取返回的 `session_id` 和 `platform`
3. 均无 → SESSION_ID = `unknown`，PLATFORM = `unknown`

## 3. 若 SELECTION_BEHAVIOR = pick
...
```
若含 `pick_options` → AskUserQuestion："检测到多个候选知识点，要记录哪一条？"（仅展示 title/type，≤MAX_PICK_OPTIONS）→ 选择 → SELECTED_CANDIDATE_IDS = `<所选 id>`；取消 → 停止。

否则 SELECTED_CANDIDATE_IDS = `none`。

## 4. Spawn note-writer

读 `{SKILL_DIR}/scripts/merge-config.ts --platform <PLATFORM>`（若 PLATFORM 非 `unknown`）→ 取 `subagent_resolved_model`。否则 MODEL 兜底 `haiku`。

| 平台 | 机制 | 参数 |
|---|---|---|
| Claude Code | `subagent_type="note-distill:note-writer"` | `model=<MODEL>`, `mode="bypassPermissions"`, `run_in_background=true` |
| CodeBuddy | Task 工具 | `subagent_name="note-writer"`, `run_in_background=true` |

> CodeBuddy：Task 工具不支持运行时指定 model。Subagent 继承主会话的模型，修改需切换主会话模型后执行 /note。
