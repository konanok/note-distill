---
name: note
description: 把当前会话中的技术方案派发到后台 subagent，写入配置的知识库。支持指定模板（如 til、design、technical）。
...
- 取第一个 token，检查是否为已知模板名。已知模板名 = 从 `<templates_dir>` 和 `{SKILL_DIR}/templates/` 两个目录下列出的 `.md` 文件名（去后缀）的合集。
- 若匹配 → TEMPLATE=该 token，剩余文本为 TOPIC_HINT
- 若不匹配 → TEMPLATE=config 的 `default_template`（缺失则 `til`），全部文本为 TOPIC_HINT
...
## 1.5 确定候选选择行为（SELECTION_BEHAVIOR）
...
合并规则：顶层字段浅层覆盖；已知嵌套对象（`candidate_selection`、`candidate_analyzer`）递归合并。
...
### 4.1 Primary path：显式输入、普通 subagent
...
Primary path 的 prompt 必须明确：subagent 不应假设拥有完整主会话历史；主要素材只来自 `NOTE_CANDIDATES` / `NOTE_EVENT_WINDOW`。

### 4.2 Fallback path：完整历史 fork

仅当 `NOTE_CANDIDATES = unavailable` 且 `NOTE_EVENT_WINDOW = unavailable` 时：

- `subagent_type="general-purpose"`
- `run_in_background=true`
- `description="写笔记"`

`CLAUDE_CODE_FORK_SUBAGENT=1` 是实验/兼容路径。

**SKILL_DIR**：从 Skill 工具载入时获取的 "Base directory for this skill"。
...
如果 NOTE_CANDIDATES 中的候选包含 `source_refs`，可使用 `node --experimental-strip-types {SKILL_DIR}/../../hooks/note_distill_hook.ts context <candidate-json-path>` 读取候选指向的局部上下文。
...
# 必读文件（按顺序读）
1. 运行 `node --experimental-strip-types {SKILL_DIR}/../../hooks/note_distill_hook.ts merge-config` 获取合并后的配置
2. {SKILL_DIR}/references/note-writer-protocol.md —— 行为规范
3. 按以下优先级找到模板文件（第一个存在的即使用）：
   - <templates_dir>/<TEMPLATE>.md
   - {SKILL_DIR}/templates/<TEMPLATE>.md
   ...
   | `{{alternatives}}` | 备选方案与取舍 | AI 生成 |
   | `{{boundaries}}` | 边界与陷阱 | AI 生成 |
   | `{{verification}}` | 验证证据 | AI 生成 |
   | `{{related}}` | 关联条目 | AI 生成 |
   | `{{scenario}}` | 场景描述 | AI 生成 |
   | `{{notes}}` | 备注 | AI 生成（可为空） |
   | `{{core_argument}}` | 核心观点 | AI 生成 |
   | `{{evidence}}` | 论据与支撑 | AI 生成 |
   | `{{counterarguments}}` | 边界与反例 | AI 生成 |
   | `{{extensions}}` | 延伸方向（TIL） | AI 生成（可为空） |
...
5. 根据 merge-config 输出的 config 中 adapter 字段读写入规范：
   - obsidian → {SKILL_DIR}/adapters/obsidian.md
   - local-markdown → {SKILL_DIR}/adapters/local-markdown.md

   **输出路径**：`<output_dir>/<filename>.md`（直接写入 output_dir，不再分子目录）。

# 核心约束
- **以本次显式输入范围为准，不要用你的训练数据替代。** 如果 NOTE_CANDIDATES 可用，以 pending candidates 为主要素材；否则如果 NOTE_EVENT_WINDOW 可用，以其中的事件窗口为主要素材；否则以增量范围内的对话历史为准。
- 按模板结构重新组织，严禁流水账式转述。
- **涉及官方文档、API 规范或可查证的事实，必须在笔记中附上来源 URL。**
...
> 📝 笔记任务已派发到后台（模板: {template}）。完成后会通知你。
